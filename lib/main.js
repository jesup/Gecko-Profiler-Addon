...
function stop_profiler(forceCache, save_callback) {
  if (forceCache || prefs.get_pref_bool("profiler.", "copyProfileOnStop", true)) {
    DEBUGLOG("Copying profile on Stop");
    var profile = null;
    profiler.getProfile(function(profile) {
      profile.meta = profile.meta || {};
      profile.meta.addons = [];
      // Let users set profiler.username if they want to tag
      // the profile with their identify. This is entirely optional.
      profile.meta.username = prefs.get_pref_string("profiler.", "username", "Anonymous");
      profile.meta.description = prefs.get_pref_string("profiler.", "description", "");
...
function sps_startup(force, manifest) {
    ...
    prefs.default_init_pref("profiler.", "threads", false);
    ...
    prefs.default_init_pref("profiler.", "mainthreadio", true);
    ...
    var want_feature_mainthreadio = prefs.get_pref_bool("profiler.", "mainthreadio", true);
    var wants_feature_copy_on_save = prefs.get_pref_bool("profiler.", "copyProfileOnStop", true);
    threadFilter = prefs.get_pref_string('profiler.', 'threadfilter', '');
    ...
    wants_feature_copy_on_save = manifest.copyProfileOnStop || wants_feature_copy_on_save;
...
exports.test = {
  ...
  stop: function Test_stop(stop_callback) {
    appWrapper.open_cleopatra();
    //stop_profiler(null, function stopped_test() {
    //  dump("stop\n");
